      ...
      const processPromises = batch.map(async (apiData) => {
        ...
        try {
          ...
          await prisma.$transaction(async (tx) => {
            ...
            if (!isUp) {
              console.warn(
                `API is down: ${url} (Status: ${response.status}, Code: ${response.statusCode})`,
              );
              const lastResponses = await tx.apiResponse.findMany({
                where: { apiId: id },
                take: 3,
                orderBy: { createdAt: "desc" },
              });
              let downCount = 0;
              for (const res of lastResponses) {
                if (res.status !== apiStatusEnum.UP) {
                  downCount++;
                }
              }
              if (downCount >= 3) {
